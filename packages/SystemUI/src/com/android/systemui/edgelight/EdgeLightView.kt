...
class EdgeLightView(context: Context) : FrameLayout(context) {
    ...
    var userStrokeWidth: Int = 8
        set(value) {
            ...
            baseStrokeWidth = field * resources.displayMetrics.density
    ...
    var animationEffect: String = EFFECT_NONE
        set(value) {
            if (field != value) {
                field = value
                stopEffectAnimation()
                if (value != EFFECT_NONE && visible) {
                    startEffectAnimation()
                }
            }
        }

    private var baseStrokeWidth: Float = 8f * resources.displayMetrics.density
    ...
    private val chasers = mutableListOf<Chaser>()
    ...
    private fun drawRoundedEdges(canvas: Canvas) {
        ...
        roundedRect.set(halfStroke, halfStroke, width.toFloat() - halfStroke, height.toFloat() - halfStroke)
        roundedPath.reset()
    ...
    private fun applyBreathingEffect() {
        ...
        val normalizedCycle = (cycle + 1f) / 2f // 0 to 1
        
        edgePaint.alpha = (155 + (normalizedCycle * 100)).toInt().coerceIn(0, 255)
        edgePaint.strokeWidth = baseStrokeWidth * (0.7f + normalizedCycle * 0.6f)
        edgePaint.maskFilter = null
    }
    ...
    private fun drawWaveEffectRounded(canvas: Canvas) {
        ...
        val measure = android.graphics.PathMeasure(roundedPath, false)
        pathLength = measure.length
        val segments = 100
        val waveAmplitude = baseStrokeWidth * 0.5f
    ...
    private fun drawSparkleEffect(canvas: Canvas) {
        ...
        if (sparkles.size < 15 && Random.nextFloat() < 0.3f) {
            ...
            sparkles.add(Sparkle(
                ...
                lifetime = 1f,
                maxSize = baseStrokeWidth * (2f + Random.nextFloat() * 2f)
            ))
        }
    ...
    private fun drawChaseEffect(canvas: Canvas) {
        ...
        for (i in 0 until numChasers) {
            ...
            val gradient = LinearGradient(
                ...
                0f, centerY + segmentLength,
                intArrayOf(0x00000000 or (edgePaint.color and 0x00FFFFFF), edgePaint.color, 0x00000000 or (edgePaint.color and 0x00FFFFFF)),
            ...
            canvas.drawLine(halfStroke, (centerY - segmentLength).coerceAtLeast(0f),
                halfStroke, (centerY + segmentLength).coerceAtMost(height.toFloat()), chaserPaint)
            canvas.drawLine(width - halfStroke, (centerY - segmentLength).coerceAtLeast(0f),
                width - halfStroke, (centerY + segmentLength).coerceAtMost(height.toFloat()), chaserPaint)
        }
    }
    ...
    private fun drawCometEffect(canvas: Canvas) {
        ...
        val gradient = LinearGradient(
            0f, (cometY - tailLength).coerceAtLeast(0f),
            0f, cometY,
        ...
        canvas.drawLine(halfStroke, (cometY - tailLength).coerceAtLeast(0f), halfStroke, cometY, cometPaint)
        canvas.drawLine(width - halfStroke, (cometY - tailLength).coerceAtLeast(0f), width - halfStroke, cometY, cometPaint)
    }
    ...
    private fun updateRainbowGradient() {
        ...
        edgePaint.shader = when (edgeStyle) {
            STYLE_ROUNDED -> {
                ...
                val gradient = SweepGradient(width / 2f, height / 2f, colors, null)
                gradient.setLocalMatrix(matrix)
                gradient
            }
            else -> {
                val offset = (rainbowRotation / 360f) * height
                LinearGradient(0f, -offset, 0f, height.toFloat() - offset, colors, null, Shader.TileMode.REPEAT)
            }
        }
    }
